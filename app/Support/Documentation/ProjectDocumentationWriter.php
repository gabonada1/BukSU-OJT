...
class ProjectDocumentationWriter
{
    ...
    protected function content(): string
    {
        ...
        return implode("\n", [
            ...
            '',
            '- Uses the tenant connection `'.config('tenancy.tenant_connection').'` after college resolution.',
            '- Main responsibilities:',
            ...
            '- Base domain for generated college subdomains: `'.config('tenancy.base_domain').'`.',
            ...
            '### BukSU University Admin',
            '',
            '- Email: `'.env('CENTRAL_SUPERADMIN_EMAIL', 'superadmin@lvh.me').'`',
    ...
    protected function tenants(): Collection
    {
        try {
            ...
            $this->warning = 'The central database was unavailable while generating this documentation. Static architecture content was written, but the live college directory could not be refreshed.';
    ...
    {
        if ($tenants->isEmpty()) {
            return 'No colleges are currently registered.';
        }
        ...
        foreach ($tenants as $tenant) {
            $rows[] = sprintf(
                ...
                $tenant->domains->pluck('host')->join(', ') ?: 'n/a',
                $tenant->database,
